...
impl MuxProber {
    ...
    pub fn analyze(&self) -> MuxAnalysis {
        ...
        let has_igpu = results
            .get("IGPU_PCI")
            .map(|r| r.status == ProbeStatus::Success)
            .unwrap_or(false);
        let has_dgpu = results
            .get("DGPU_PCI")
            .map(|r| r.status == ProbeStatus::Success)
            .unwrap_or(false);
        ...
        if has_igpu && has_dgpu && !analysis.has_mux_switch {
            analysis.recommendations.push(
                "Both GPUs detected but no MUX switch found. \
                 This laptop may have a hardwired hybrid design (no MUX)."
                    .to_string(),
            );
        }

        if analysis.has_mux_switch {
            analysis.risk_assessment = "MUX switch found. Toggling requires reboot. \
                 Risk: LOW if using vendor software, MEDIUM if using EC writes."
                .to_string();

            analysis
                .recommendations
                .push("Try vendor software first (Armoury Crate, Vantage, etc.)".to_string());
            analysis.recommendations.push(
            ...
            analysis
                .recommendations
                .push("Use GPU orchestrator to offload tasks to iGPU instead".to_string());
            analysis
                .recommendations
                .push("Consider external GPU dock for true bypass".to_string());
        }
...
fn probe_location(loc: &ProbeLocation) -> ProbeResult {
    ...
    let (status, value, interpretation) = match loc.method {
        ...
        ProbeMethod::IoPort => (
            ProbeStatus::Skipped,
            None,
            Some("I/O port access disabled".to_string()),
        ),
        ProbeMethod::PhysicalMemory => (
            ProbeStatus::Skipped,
            None,
            Some("Physical memory access requires driver".to_string()),
        ),
        ProbeMethod::SmBios => probe_smbios(),
        ProbeMethod::EcWrite => (
            ProbeStatus::Skipped,
            None,
            Some("Write operations disabled".to_string()),
        ),
        ProbeMethod::NvramRead => (
            ProbeStatus::Skipped,
            None,
            Some("NVRAM access disabled".to_string()),
        ),
    };
...
fn probe_wmi(name: &str) -> (ProbeStatus, Option<u64>, Option<String>) {
    ...
    if name.contains("ASUS") {
        ...
        (
            ...
            None,
            Some("ASUS WMI not present".to_string()),
        )
    } else if name.contains("LENOVO") {
        (
            ProbeStatus::NotFound,
            None,
            Some("Lenovo WMI not present".to_string()),
        )
    } else {
...
fn probe_ec_ram(offset: u8) -> (ProbeStatus, Option<u64>, Option<String>) {
    ...
    (
        ...
        None,
        Some("EC access requires kernel driver".to_string()),
    )
}
...
fn probe_msr(msr: u32) -> (ProbeStatus, Option<u64>, Option<String>) {
    ...
    (
        ProbeStatus::AccessDenied,
        None,
        Some("MSR access requires kernel driver".to_string()),
    )
}
...
fn probe_smbios() -> (ProbeStatus, Option<u64>, Option<String>) {
    ...
    (
        ProbeStatus::Success,
        None,
        Some("SMBIOS tables accessible".to_string()),
    )
}
...
fn probe_wmi(_name: &str) -> (ProbeStatus, Option<u64>, Option<String>) {
    (
        ProbeStatus::NotFound,
        None,
        Some("WMI is Windows-only".to_string()),
    )
}
...
fn probe_ec_ram(_offset: u8) -> (ProbeStatus, Option<u64>, Option<String>) {
    ...
    (
        ProbeStatus::AccessDenied,
        None,
        Some("EC debugfs requires root".to_string()),
    )
}
...
fn probe_msr(_msr: u32) -> (ProbeStatus, Option<u64>, Option<String>) {
    ...
    (
        ProbeStatus::AccessDenied,
        None,
        Some("MSR requires root".to_string()),
    )
}
...
impl MuxMcpServer {
    ...
    fn handle_probe(
        ...
        serde_json::to_value(results).map_err(|e| McpError {
            code: -32603,
            message: e.to_string(),
        })
    }
    ...
    fn handle_progress(&self) -> Result<serde_json::Value, McpError> {
        ...
        serde_json::to_value(progress).map_err(|e| McpError {
            code: -32603,
            message: e.to_string(),
        })
    }
    ...
    fn handle_analyze(&self) -> Result<serde_json::Value, McpError> {
        ...
        serde_json::to_value(analysis).map_err(|e| McpError {
            code: -32603,
            message: e.to_string(),
        })
    }
    ...
    fn handle_list_locations(&self) -> Result<serde_json::Value, McpError> {
        let locations = get_known_mux_locations();
        serde_json::to_value(locations).map_err(|e| McpError {
            code: -32603,
            message: e.to_string(),
        })
    }
...
static MUX_SERVER: Lazy<RwLock<MuxMcpServer>> = Lazy::new(|| RwLock::new(MuxMcpServer::new()));
