...
fn detect_nvidia_gpus() -> Option<Vec<GpuDevice>> {
    ...
    // Initialize NVML
    let init: Symbol<unsafe extern "C" fn() -> i32> = unsafe { nvml.get(b"nvmlInit_v2").ok()? };
...
fn detect_amd_gpus() -> Option<Vec<GpuDevice>> {
    ...
    let create: Symbol<
        unsafe extern "C" fn(extern "C" fn(i32) -> *mut std::ffi::c_void, i32) -> i32,
    > = unsafe { adl.get(b"ADL_Main_Control_Create").ok()? };
...
impl GpuOrchestrator {
    ...
    pub fn route_task(&self, task_type: TaskType) -> Option<u32> {
        ...
        match preference {
            ...
            GpuPreference::LeastLoaded => gpus
                .iter()
                .min_by(|a, b| {
                    a.current_load_percent
                        .partial_cmp(&b.current_load_percent)
                        .unwrap()
                })
                .map(|g| g.id),
            GpuPreference::MostCapable => gpus.iter().max_by_key(|g| g.compute_units).map(|g| g.id),
...
static ORCHESTRATOR: Lazy<GpuOrchestrator> = Lazy::new(|| GpuOrchestrator::new());
...
pub mod vcn {
    ...
    pub fn vcn_available() -> bool {
        ...
        gpus.iter()
            .any(|g| g.vendor == GpuVendor::Amd && g.has_video_encode)
    }
...
pub async fn honeypot_generate(decoy_type: String, width: u32, height: u32) -> Vec<u8> {
    ...
    thread::spawn(|| {
        ...
        loop {
            ...
            let secondary_active = tasks.values().any(|t| {
                Some(t.assigned_gpu) == config.secondary_gpu_id && t.status == TaskStatus::Running
            });
