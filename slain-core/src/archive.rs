...
fn add_file_to_zip<W: Write + Seek>(
    ...
    let mut file = File::open(path).map_err(|e| format!("Failed to open {}: {}", name, e))?;

    zip.start_file(name, *options)
        .map_err(|e| format!("Failed to start file: {}", e))?;

    let written = io::copy(&mut file, zip).map_err(|e| format!("Failed to write file: {}", e))?;
...
pub fn list_tar<P: AsRef<Path>>(path: P) -> Result<ArchiveInfo, String> {
    ...
    let reader: Box<dyn Read> = match format {
        ArchiveFormat::TarGz | ArchiveFormat::Gz => Box::new(GzDecoder::new(file)),
        _ => Box::new(file),
    };
...
pub fn create_tar_gz<P: AsRef<Path>>(
    ...
    for source in source_paths {
        ...
        if source.is_dir() {
            ...
            let mut file = File::open(source).map_err(|e| format!("Failed to open file: {}", e))?;
...
pub fn compress_gzip<P: AsRef<Path>, Q: AsRef<Path>>(
    ...
    let input = File::open(input_path).map_err(|e| format!("Failed to open input: {}", e))?;
    ...
    io::copy(&mut input, &mut encoder).map_err(|e| format!("Failed to compress: {}", e))?;
...
pub fn decompress_gzip<P: AsRef<Path>, Q: AsRef<Path>>(
    ...
    io::copy(&mut decoder, &mut output).map_err(|e| format!("Failed to decompress: {}", e))?;
...
pub async fn archive_extract(
    ...
    match format {
        ...
        ArchiveFormat::Tar | ArchiveFormat::TarGz => {
            extract_tar(&archive_path, &output_dir, entries)
        }
        ArchiveFormat::Gz => {
