...
fn get_cuda_lib_path() -> String { String::new() }

#[cfg(not(any(target_os = "windows", target_os = "linux")))]
fn get_cuvid_lib_path() -> String { String::new() }
...
type CuvidCreateVideoParserFn = unsafe extern "C" fn(*mut CUvideoparser, *mut CuvidParserParams) -> CUresult;
type CuvidDestroyVideoParserFn = unsafe extern "C" fn(CUvideoparser) -> CUresult;
type CuvidParseVideoDataFn = unsafe extern "C" fn(CUvideoparser, *mut CuvidSourceDataPacket) -> CUresult;
type CuvidCreateDecoderFn = unsafe extern "C" fn(*mut CUvideodecoder, *mut CuvidDecodeCreateInfo) -> CUresult;
...
type CuvidMapVideoFrameFn = unsafe extern "C" fn(CUvideodecoder, i32, *mut CUdeviceptr, *mut u32, *mut CuvidProcParams) -> CUresult;
    ...
    NVDEC_LIBS.get_or_init(|| {
        #[cfg(any(target_os = "windows", target_os = "linux"))]
        {
            unsafe {
                ...
                let cu_device_get_count: CuDeviceGetCountFn = *cuda_lib.get(b"cuDeviceGetCount\0").ok()?;
                let cu_device_get_name: CuDeviceGetNameFn = *cuda_lib.get(b"cuDeviceGetName\0").ok()?;
                let cu_device_get_attribute: CuDeviceGetAttributeFn = *cuda_lib.get(b"cuDeviceGetAttribute\0").ok()?;
                let cu_ctx_create: CuCtxCreateFn = *cuda_lib.get(b"cuCtxCreate_v2\0").ok()?;
                let cu_ctx_destroy: CuCtxDestroyFn = *cuda_lib.get(b"cuCtxDestroy_v2\0").ok()?;
                let cu_ctx_push_current: CuCtxPushCurrentFn = *cuda_lib.get(b"cuCtxPushCurrent_v2\0").ok()?;
                let cu_ctx_pop_current: CuCtxPopCurrentFn = *cuda_lib.get(b"cuCtxPopCurrent_v2\0").ok()?;
                let cu_memcpy_dtoh: CuMemcpyDtoHFn = *cuda_lib.get(b"cuMemcpyDtoH_v2\0").ok()?;
                
                // Load CUVID functions
                let cuvid_create_video_parser: CuvidCreateVideoParserFn = *cuvid_lib.get(b"cuvidCreateVideoParser\0").ok()?;
                let cuvid_destroy_video_parser: CuvidDestroyVideoParserFn = *cuvid_lib.get(b"cuvidDestroyVideoParser\0").ok()?;
                let cuvid_parse_video_data: CuvidParseVideoDataFn = *cuvid_lib.get(b"cuvidParseVideoData\0").ok()?;
                let cuvid_create_decoder: CuvidCreateDecoderFn = *cuvid_lib.get(b"cuvidCreateDecoder\0").ok()?;
                let cuvid_destroy_decoder: CuvidDestroyDecoderFn = *cuvid_lib.get(b"cuvidDestroyDecoder\0").ok()?;
                let cuvid_decode_picture: CuvidDecodePictureFn = *cuvid_lib.get(b"cuvidDecodePicture\0").ok()?;
                let cuvid_map_video_frame: CuvidMapVideoFrameFn = *cuvid_lib.get(b"cuvidMapVideoFrame64\0")
                    .or_else(|_| cuvid_lib.get(b"cuvidMapVideoFrame\0")).ok()?;
                let cuvid_unmap_video_frame: CuvidUnmapVideoFrameFn = *cuvid_lib.get(b"cuvidUnmapVideoFrame64\0")
                    .or_else(|_| cuvid_lib.get(b"cuvidUnmapVideoFrame\0")).ok()?;
    ...
    unsafe {
        ...
        let mut create_info = CuvidDecodeCreateInfo {
            ...
            display_area: CuvidRect {
                ...
                right: if fmt.display_area.right > 0 { fmt.display_area.right } else { fmt.coded_width as i16 },
                bottom: if fmt.display_area.bottom > 0 { fmt.display_area.bottom } else { fmt.coded_height as i16 },
            },
...
pub fn nvdec_capabilities() -> NvdecCapabilities {
    ...
    unsafe {
        ...
        if major >= 5 { codecs.push("H.265".to_string()); }
        if major >= 6 { codecs.push("VP9".to_string()); }
        if major >= 8 { codecs.push("AV1".to_string()); }
...
impl NvdecDecoder {
    ...
    pub fn new(codec: VideoCodec, width: u32, height: u32) -> Result<Self, String> {
        let libs = load_nvdec_libraries()
            .ok_or_else(|| "NVDEC not available".to_string())?;
    ...
    fn map_pending_frame(&self) -> Result<Option<DecodedFrame>, String> {
        ...
        unsafe {
            ...
            let result = (self.libs.cuvid_map_video_frame)(
                state.decoder, pending.picture_index,
                &mut device_ptr, &mut pitch, &mut proc_params,
            );
            ...
            let format = if state.bit_depth > 8 { FrameFormat::P016 } else { FrameFormat::NV12 };
