...
use crate::forumyze::{ForumyzeResult, YouTubeComment, Discussion};
...
impl CourtListenerClient {
    ...
    pub async fn search_case_law(&self, query: &str, limit: usize) -> Result<Vec<CaseLawReference>, String> {
        let url = format!("{}/search/?q={}&type=o", self.base_url, urlencoding::encode(query));
        ...
        let cases: Vec<CaseLawReference> = data.results.into_iter()
            .take(limit)
            .filter_map(|r| {
                Some(CaseLawReference {
                    ...
                    key_holdings: vec![],
                    url: format!("https://www.courtlistener.com{}", r.absolute_url.unwrap_or_default()),
    ...
    fn build_legal_queries(&self, theme: &str) -> Vec<String> {
        ...
        if theme_lower.contains("death") || theme_lower.contains("killed") || theme_lower.contains("shot") {
        ...
        if theme_lower.contains("choke") || theme_lower.contains("restrain") || theme_lower.contains("knee") {
...
impl EvidencePackageBuilder {
    ...
    pub async fn build(&self, forumyze: &ForumyzeResult, case_reference: Option<String>) -> EvidencePackage {
        ...
        let video_evidence = VideoEvidence {
            ...
            url: format!("https://www.youtube.com/watch?v={}", forumyze.video.video_id),
            ...
            archive_urls: vec![
                format!("https://web.archive.org/web/https://www.youtube.com/watch?v={}", forumyze.video.video_id),
            ],
            content_hash: None,
        };
    ...
    fn comment_to_witness_statement(&self, comment: &YouTubeComment, base_relevance: f32) -> Option<WitnessStatement> {
    ...
    fn classify_statement(&self, text: &str) -> StatementClassification {
        ...
        if text_lower.contains("i saw") || text_lower.contains("i witnessed") 
            || text_lower.contains("i was there") || text_lower.contains("i watched it happen") {
            return StatementClassification::Eyewitness;
        }
        
        // First-hand indicators
        if text_lower.contains("i know") || text_lower.contains("i live") 
            || text_lower.contains("my neighbor") || text_lower.contains("my friend")
            || text_lower.contains("i work") {
            return StatementClassification::FirstHand;
        }
        
        // Expert indicators
        if text_lower.contains("as a lawyer") || text_lower.contains("as a doctor")
            || text_lower.contains("as a nurse") || text_lower.contains("as a cop")
            || text_lower.contains("former officer") || text_lower.contains("legal expert") {
        ...
        // Second-hand
        if text_lower.contains("heard that") || text_lower.contains("someone said")
            || text_lower.contains("apparently") || text_lower.contains("reportedly") {
            return StatementClassification::SecondHand;
        }
        
        // Emotional
        let emotional_words = ["rip", "rest in peace", "so sad", "heartbreaking", "prayers", "justice"];
    ...
    fn extract_claims(&self, text: &str) -> Vec<ExtractedClaim> {
        ...
        let observation_patterns = [
            "i saw", "i watched", "i witnessed", "you can see", "the video shows",
            "clearly shows", "footage shows",
        ];
        ...
        // Temporal claims
        let time_patterns = [
            "at ", "around ", "minutes before", "minutes after", "seconds",
            "o'clock", "am", "pm",
        ];
        ...
        let contradiction_patterns = [
            "but actually", "that's not true", "they're lying", "the truth is",
            "what really happened", "they left out", "didn't show",
        ];
    ...
    fn is_first_hand(&self, text: &str) -> bool {
        let indicators = [
            "i saw", "i was there", "i witnessed", "i know him", "i know her",
            "i live", "my neighborhood", "my community", "i work",
        ];
        let text_lower = text.to_lowercase();
    ...
    fn analyze_sentiment(&self, text: &str) -> f32 {
        ...
        let negative = ["murder", "killed", "wrong", "evil", "corrupt", "injustice", "disgusting"];
    ...
    fn extract_locations(&self, text: &str) -> Vec<String> {
        ...
        for window in words.windows(2) {
            ...
            if phrase.contains("Street") || phrase.contains("Ave") 
                || phrase.contains("Boulevard") || phrase.contains("Road") {
    ...
    fn build_thematic_analysis(&self, forumyze: &ForumyzeResult) -> ThematicAnalysis {
        let themes: Vec<Theme> = forumyze.discussions.iter().map(|d| {
            Theme {
                id: d.id.clone(),
                name: d.title.clone(),
        ...
        let themes = if themes.is_empty() {
            forumyze.topics.iter().enumerate().map(|(i, topic)| {
                Theme {
                    id: format!("topic_{}", i),
                    name: topic.clone(),
    ...
    fn get_date_range(&self, statements: &[WitnessStatement]) -> Option<(String, String)> {
        ...
        let timestamps: Vec<&str> = statements.iter()
    ...
    fn calc_avg_engagement(&self, forumyze: &ForumyzeResult) -> f32 {
        ...
        if count == 0 { 0.0 } else { total_likes as f32 / count as f32 }
    }
...
impl EvidencePackage {
    ...
    pub fn to_markdown(&self) -> String {
        ...
        md.push_str("---\n\n## Statistics\n\n");
        md.push_str(&format!("- Total comments analyzed: {}\n", self.statistics.total_comments_analyzed));
        md.push_str(&format!("- Witness statements extracted: {}\n", self.statistics.witness_statements_extracted));
        md.push_str(&format!("- First-hand accounts: {}\n", self.statistics.first_hand_accounts));
        md.push_str(&format!("- Unique authors: {}\n", self.statistics.unique_authors));
        md.push_str(&format!("- Related case law: {}\n\n", self.statistics.case_law_references));
        
        md.push_str("---\n\n## Key Witness Statements\n\n");
        for (i, stmt) in self.witness_statements.iter().take(20).enumerate() {
            md.push_str(&format!("### {}. {} ({:?})\n\n", i + 1, stmt.author, stmt.classification));
            md.push_str(&format!("> {}\n\n", stmt.statement));
            md.push_str(&format!("*Likes: {} | First-hand: {} | Relevance: {:.2}*\n\n", 
