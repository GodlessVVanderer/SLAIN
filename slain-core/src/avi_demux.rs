...
impl<R: Read + Seek> AviDemuxer<R> {
    ...
    fn parse_avih(&mut self, _size: u32) -> Result<(), String> {
        ...
        if header.microsec_per_frame > 0 {
            ...
            self.info.duration_us =
                (header.total_frames as i64) * (header.microsec_per_frame as i64);
        }
        ...
        while self.reader.stream_position().unwrap() < end {
            ...
            match fourcc {
                STRH => {
                    ...
                    stream_info.codec =
                        identify_codec(stream_header.fcc_handler, stream_info.stream_type);

                    if stream_header.scale > 0 && stream_header.rate > 0 {
                        stream_info.fps =
                            Some(stream_header.rate as f64 / stream_header.scale as f64);
                    }
                ...
                STRF => match stream_info.stream_type {
                    StreamType::Video => {
    ...
    fn parse_idx1(&mut self, size: u32) -> Result<(), String> {
        ...
        for _ in 0..entries {
            ...
            let stream_id =
                ((chunk_id & 0xFF) - b'0' as u32) * 10 + (((chunk_id >> 8) & 0xFF) - b'0' as u32);
