...
impl WatchHistory {
    ...
    pub fn update_progress(&mut self, video_id: &str, position: f64, duration: f64, title: &str) {
        let progress = self.progress.entry(video_id.to_string()).or_insert_with(|| {
            WatchProgress::new(video_id, title, duration)
        });
    ...
    pub fn get_continue_watching(&self, limit: usize) -> Vec<&WatchProgress> {
        let mut items: Vec<_> = self.progress.values()
            .filter(|p| p.should_resume())
            .collect();
...
static HISTORY: Lazy<Mutex<WatchHistory>> = Lazy::new(|| {
    Mutex::new(WatchHistory::load().unwrap_or_default())
});
...
pub fn get_continue_watching() -> Vec<WatchProgress> {
    ...
    history.get_continue_watching(20)
        .into_iter()
        .cloned()
        .collect()
}
...
pub fn get_recently_watched() -> Vec<WatchProgress> {
    ...
    history.get_recently_watched(50)
        .into_iter()
        .cloned()
        .collect()
}
...
pub fn add_video_bookmark(video_id: String, position: f64, label: String) -> Result<Bookmark, String> {
    let mut history = HISTORY.lock().map_err(|e| e.to_string())?;
...
pub fn get_video_bookmarks(video_id: String) -> Vec<Bookmark> {
    ...
    history.get_bookmarks(&video_id)
        .into_iter()
        .cloned()
        .collect()
}
