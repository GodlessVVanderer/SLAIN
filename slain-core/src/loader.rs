...
const VULKAN_PATHS: &[&str] = &["C:\\Windows\\System32", "C:\\Windows\\SysWOW64"];
...
pub fn load_bundled(name: &str) -> Result<LoadedLibrary, String> {
    ...
    let metadata =
        std::fs::metadata(&path).map_err(|e| format!("Failed to read {}: {}", name, e))?;
...
pub fn check_nvidia_libraries() -> Vec<LibraryStatus> {
    ...
    libs.iter()
        .map(|(name, _desc)| match load_nvidia(name) {
            Ok(lib) => {
